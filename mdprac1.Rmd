---
- title:CoMo Refugee Report author:Yutong output:html_docutment
---
...
head(df)
ggplot(df, aes(YEAR))+
  geom_line(aes(y = COLUMBIA, color = 'COLUMBIA'))+
  geom_line(aes(y = MISSOURI, color = 'MISSOURI'))+
  theme_fivethirtyeight()+
  scale_color_fivethirtyeight()+
  ggtitle("2002-2018 Refugee Arrivals - Columbia and Missouri")+
...
![Where are people who resettled in Columbia from?](~/Documents/GitHub/Rpractice/Columbia_Refugee_Map.png)
...
```{r echo = False}
df <- read.csv('columbia_refugee_nationality.csv', stringsAsFactors = FALSE)
                                                                        
ggplot(df, aes(x=nationality, y=number) ) + 
  geom_bar(stat = "identity",fill = "#008FD5", width = 0.8)+ 
  coord_cartesian(ylim=c(0,400))+
  coord_flip()+
  theme_fivethirtyeight()+
  ggtitle("2002-2018 Refugee Arrivals by Nationality")
```
